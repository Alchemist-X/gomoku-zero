...
class SearchSession:
        ...
        for index, raw_move in enumerate(root_moves):
            ...
            move_items.append(
                MoveStats(
                    ...
                    visits=visits,
                    visit_fraction=(
                        visits / total_child_visits if total_child_visits else 0.0
                    ),
    ...
    def _expand(self, node: _Node) -> FloatArray:
        ...
        raw_policy, raw_wdl = self._call_evaluator(board)
    ...
    def _simulate(self) -> None:
        ...
        value = _value_for_player(leaf_wdl, int(_property_value(node.board, "to_play")))
        for child in reversed(path):
            value = -value
            child.visit_count += 1
            child.value_sum += value
            child.black_wdl_sum += float(leaf_wdl[0])
            child.draw_wdl_sum += float(leaf_wdl[1])
            child.white_wdl_sum += float(leaf_wdl[2])
